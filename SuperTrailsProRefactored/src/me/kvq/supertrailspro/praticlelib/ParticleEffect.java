...
import me.kvq.supertrailspro.praticlelib.ReflectionUtils.PackageType;
...
public enum ParticleEffect {
	...
	END_ROD("endrod", 43, -1 , ParticleProperty.DIRECTIONAL),
	
	DAMAGE("damageindicator", 44, -1 , ParticleProperty.DIRECTIONAL),
	
	FALLING_DUST("fallingdust", 46, -1, ParticleProperty.DIRECTIONAL),
	
	Spit("spit", 47, -1, ParticleProperty.DIRECTIONAL),
	
	Totem("totem", 47, -1, ParticleProperty.DIRECTIONAL),
	
	Soul("soul", 47, -1,  ParticleProperty.DIRECTIONAL),
	Soul_Fire("soul_fire", 47, -1,  ParticleProperty.DIRECTIONAL),
	NAUTILUS("NAUTILUS",47,-1, ParticleProperty.DIRECTIONAL);
	...
	public void display(Location l, double range) {
		display(0f, 0f, 0f, 0f, 3, l, range);
	}
	...
	public static final class OrdinaryColor extends ParticleColor {
		...
		public OrdinaryColor clone() {	
			return new OrdinaryColor(red, green, blue);
		}
	...
	public static final class ParticlePacket {
		...
		public static void initialize() throws VersionIncompatibleException {
			...
			try {
				...
				if (ServerVersion.atleast(Version.MC10)) version = 8;
				if (version > 7) {
					enumParticle = PackageType.MINECRAFT_SERVER.getClass("EnumParticle");
						
	//enumParticle = EnumParticle.class;
				}
		...
		private void initializePacket(Location center) throws PacketInstantiationException {
			...
			try {
				packet = packetConstructor.newInstance(); version = 8;
				if (version < 8) {
...
//					ReflectionUtils.setValue(packet, true, "a", Menu.getTrailName(effect));
//					Bukkit.broadcastMessage("" + enumParticle.getEnumConstants()[effect.getId()]);
...
//			if (SuperTrails.toggled.contains(player)) return;
			try {
		...
		public void sendTo(Location center, double range, Player from) throws IllegalArgumentException {
			...
			for (Player player : Bukkit.getOnlinePlayers()) {
				if (!player.getWorld().getName().equals(worldName) || player.getLocation().distanceSquared(center) > squared) {
					...
					//Bukkit.broadcastMessage("" + from.getName() + " see " + player.getName() + " : " + from.canSee(player));
					sendTo(center, player);
		 ...
		 * <p>
		 * This class is part of ethe <b>ParticleEffect Library</b> and follows the same usage conditions
		 * 
		 * @author DarkBlade12
		 * @since 1.5
		 */
